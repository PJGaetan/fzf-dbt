...
bashProfile=$(cat ~/.zprofile | sed '/fzf_dbt.sh/d')
echo "${bashProfile}${NL}${NL}# Leave this fzf_dbt.sh incantation as a single line, so that homebrew upgrades are smooth${NL}if [ -f $(brew --prefix)/etc/fzf_dbt.sh ]; then source $(brew --prefix)/etc/fzf_dbt.sh; fi${NL}" > ~/.bash_profile
